...
<div class="row-fluid">
        ...
        <div class="well ticket">
            ...
            <div class="content">
                <?php if (!$isRepeater and $ticket->getRealSender()): ?>
                    <div class="alert alert-info">Письмо пришло от <a
                                href="mailto:<?php echo $ticket->getRealSender() ?>"><?php echo $ticket->getRealSender() ?></a>.
                    </div>
                <?php endif ?>
                <h1 class="page-header">
                    ...
                    <?php if (!$isRepeater): ?>
                        ...
                        <?php elseif (
                            !$ticket->getIsClosed() and $sf_user->getGuardUser()->getType() === 'it-admin'
                            and Helpdesk::checkIfImInList($sf_user->getRawValue()->getGuardUser(),
                                $ticket->getRawValue()->getResponsibles())
                        ): ?>
                            <a href="<?php echo url_for('tickets/apply?id=' . $ticket->getId()) ?>" class="btn">принять
                                в работу</a>
                        <?php else: ?>
                            ещё не обработана
                        <?php endif ?>
            ...
            <?php if (!$isRepeater and true == ($attach = $ticket->attach)): ?>
                ...
                <div class="attachment">
                    Вложения:
                    <ul>
                        <li><a href="/uploads/ticket-attachments/<?php echo $attach ?>"
                               target="_blank"><?php echo $attach ?></a></li>
                    </ul>
                </div>
            <?php endif ?>
        </div>
                ...
                <table class="table tickets20">
                    ...
                    <tbody><?php foreach ($ticket->getTickets() as $ticket_): ?>
                        <tr>
                            ...
                            <td><span class="icon icon-<?php echo $ticket_->getIsClosed() ? 'ok' : 'remove' ?>"></span>
                            </td>
                        </tr>
    ...
    <div class="span2 ticket-status">
        <div>
            Заявка №<a href="<?php echo url_for('@tickets-show?id='
                . $ticket->getId()) ?>"><?php echo $ticket->getId() ?></a>
        </div>
        ...
        <?php if ($canManipulateThisTicket): ?>
            <div class="center">
                <a href="<?php echo url_for('ticket' . ($isRepeater ? 'Repeater' : 's') . '/edit?id='
                    . $ticket->getId()) ?>" class="btn btn-mini">
        ...
        <?php if ($sf_user->hasCredential('delete_tickets')): ?>
            <div class="center">
                <a href="#" class="btn btn-mini ticket-deleter"
                   data-delete-uri="<?php echo url_for('ticket' . ($isRepeater ? 'Repeater' : 's') . '/delete?id='
                       . $ticket->getId()) ?>">
    ...
    <div class="span">
      <img width="48" src="http://placehold.it/140" alt="userpic of <?php //echo $comment->getCreator()->getUsername() ?>" />
                ...
                <div class="comment">
                    ...
                    <div class="header">
                        <span class="who"><a href="#"><?php echo ($commentCreator = $comment->getCreator()) == true
                                    ? $commentCreator->getUsername() : 'unknown' ?></a></span>
                        <span class="what">
          добавил комментарий
                    ...
                    <div class="content">
                        <?php if ($sf_user->hasCredential('can_delete_comments')): ?>
                            <ul class="actions unstyled visible-phone visible-tablet hidden-desktop">
                                <li class="pull-left">
                                    <a href="#" class="btn btn-mini" rel="tooltip" title="edit"
                                       onclick="alert('not implemented yet'); return false">
                                        <i class="icon-pencil"></i>
                                    </a>
                                </li>
                                <li class="pull-left">
                                    <a href="#" class="btn btn-mini comment-deleter" rel="tooltip" title="delete"
                                       data-delete-uri="<?php
                                       echo url_for('@comments-delete?id=' . $ticket->getId() . '&comment='
                                           . $comment->getId()) ?>">
                        ...
                        <?php endif ?>
                        <?php echo Helpdesk::replaceTicketMentionsWithLinks(Helpdesk::replaceTicketRepeaterMentionsWithLinks(simple_format_text($comment->getText()))) ?>

                        <?php if ($attachment = $comment->getAttachment()): ?><p>
                            <a href="/uploads/comment-attachments/<?php echo $attachment ?>" target="_blank"
                               class="btn btn-link">
...
<?php if (!$isRepeater): ?>
    <form action="<?php echo url_for('@comments-create?id=' . $ticket->getId()) ?>" method="post"
          class="well form-fluid add-comment" enctype="multipart/form-data">
        ...
        <div class="btn-toolbar">
            ...
            <div class="btn-group" ng-controller="TicketShowPageController">
                <?php if ($sf_user->hasCredential('can mark tickets as duplicates') and !$ticket->getIsClosed()): ?>
                    <button class="btn" ng-click="closeAsDup(<?php echo $ticket->getId() ?>)">Закрыть как дубликат
                    </button>
                <?php endif ?>

                <?php if (
                Helpdesk::checkIfImInList($sf_user->getRawValue()->getGuardUser(),
                    $ticket->getRawValue()->getResponsibles())
                ): ?>
                    <button class="btn" ng-click="iAmNotResponsibleForThis(<?php echo $ticket->getId() ?>)">Отказаться
                        от заявки
                    </button>
                <?php endif ?>
            </div>
                ...
                <?php if ($canManipulateThisTicket): ?>
                    ...
                    <?php elseif ($ticket->getCategoryId() === null): ?>
                        <div class="alert alert-info"
                             style="display: inline-block; font-size: 14px; padding: 4px 15px;">Нельзя закрыть заявку
                            без категории.
                        </div>
                    <?php elseif (!$applier): ?>
                        <div class="alert alert-info"
                             style="display: inline-block; font-size: 14px; padding: 4px 15px;">Нельзя закрыть заявку не
                            принимая её в работу.
                        </div>
                    <?php else: ?>
                ...
                <div class="modal-body">
                    <form action="">
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <textarea name="name" id="name" cols="30" rows="10" class="fluid"
                                          ng-model="reasonForDecline" required></textarea>
                            </div>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" ng-click="confirmDecline(reasonForDecline)">
                        Отказаться
                    </button>
    ...
    <div class="modal top am-fade-and-slide-top" tabindex="-1" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div className="alert alert-error" ng-show="error">Ошибка загрузки списка заявок, попробуйте ещё
                        раз
                    </div>
                    <form action="">
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <select name="name" id="name"
                                        ng-options="ticket.id as (ticket.id + ': ' + ticket.name) for ticket in tickets"
                                        ng-model="parentId" class="fluid" required>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" ng-click="confirmClose(parentId, $event)">Закрыть
                    </button>
                    <button type="button" class="btn btn-default" ng-click="$hide()">Отмена</button>
                </div>
            </div>
